...
def baliho():
  ...
  -------------       -------------------------------------
   ...
   get_data           mengambil semua data teman anda
   get_info           Mengambil informasi tentang teman Anda

   dump_id            mengambil semua id dari daftar teman
   dump_phone         mengambil semua nomor telepon dari daftar teman
   dump_mail          mengambil semua email dari daftar teman
   dump_<id>_id       mengambil semua id dari teman Anda <spesific>
	contoh: dump_username_id

   token              Generate access token
   cat_token          lihat access token
   rm_token           hapus access token

   bot                buka menu bot Facebook

   clear              clear terminal
   help               bantuan menggunakan tools ini
   about              Tampilkan informasi tentang program ini
...
* Jika Anda menemukan errora atau masalah , please contact author
'''%(G,W)
def info_ga():
...
def menu_reaction_ask():
  try:
	...
	cek = raw_input(R + 'root@Hak9:~' + W + '/' + R + 'Bot' + W + '/' + R + 'Reaction' + W + ' $ ')
...
def bot():
  try:
	...
	cek = raw_input(R + 'root@Hak9:~' + W +'/' + R +'Bot ' + W + '$ ')
...
'''
	...
	if 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
		...
		if cek == '':
			...
			print "[!] command '"+cek+"' not found"
			print '[!] ketik "help" untuk menampilkan perintah'
			main()
  except KeyboardInterrupt:
	main()
  except IndexError:
